...
function parseCosts(input: string): Costs[] {
  const fields = [
  ...
  return input.
    ...
    map(line => line.
      ...
      replace(/\D+$/, '').
      split(/\D+/).
      reduce((costs: Costs, num: string, idx: number, arr) => {
        costs[fields[idx]] = +num;
        return costs;
      }, {} as Partial<Costs>) as Costs
    ) as Costs[];
}
...
export const P = new Puzzle({
  ...
  part1: (input: string, isTest: boolean) => {
    ...
    const scores: number[] = parseCosts(input).
      map((costs: Costs, idx: number) => {
        let maxGeodes = 0;
        ...
        while (queue.length) {
          ...
          if ((states[hash(sim)] ?? 999) <= sim.minute) {
            continue;
          }
        ...
        // console.log('Blueprint ' + (idx + 1) + ': ' + maxGeodes + ' geodes. Score: ', maxGeodes * (idx + 1));
        return maxGeodes * (idx + 1);
      });
